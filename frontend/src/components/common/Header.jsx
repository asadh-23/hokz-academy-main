...
import { Heart, ShoppingCart, MessageCircle, GraduationCap } from "lucide-react";
...
export const Header = () => {
    ...
    <header
        className={`sticky top-0 left-0 w-full z-50 transition-all duration-300 ${
            isScrolled ? "bg-[#FDFDFD]/90 backdrop-blur-md shadow-lg py-3" : "bg-[#FDFDFD] lg:bg-transparent py-5"
        }`}
    >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2 group cursor-pointer shrink-0">
                    <div className="w-10 h-10 sm:w-11 sm:h-11 bg-[#1E2EDE] rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-12 shadow-xl shadow-blue-900/20">
                        <GraduationCap className="text-[#E6D929] w-6 h-6 sm:w-7 sm:h-7" />
                    </div>
                    <span className="text-xl sm:text-2xl font-black tracking-tighter text-[#1E2EDE]">
                ...
                <div className="lg:hidden flex items-center gap-2">
                    {isAuthenticated && (
                        <div className="flex items-center mr-1">
                             <button onClick={() => navigate("/user/cart")} className="p-2 text-[#1E2EDE]">
                    ...
                    <button 
                        ...
                        className="text-[#1E2EDE] p-2 focus:outline-none"
                    >
        ...
        {/* Mobile Navigation */}
        <div
            className={`lg:hidden absolute top-full left-0 w-full bg-[#FDFDFD] border-t border-slate-100 transition-all duration-300 shadow-xl overflow-hidden ${
                ...
                <nav className="flex flex-col space-y-4">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            to={link.path}
                            className="text-lg font-bold text-[#1E2EDE] hover:text-[#14C4E7]"
                    ...
                    {isAuthenticated ? (
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className="flex items-center justify-center py-3 border-2 border-[#1E2EDE] text-[#1E2EDE] rounded-xl font-bold"
                            >
                                Profile
                            </button>
                            <button
                                onClick={() => { navigate("/user/wishlist"); setIsMobileMenuOpen(false); }}
                                className="flex items-center justify-center py-3 border-2 border-[#1E2EDE] text-[#1E2EDE] rounded-xl font-bold"
                            >
                                Wishlist
                            </button>
                        ...
                        <>
                            ...
                            <Link
                                to="/user/register"
                                className="w-full py-3 bg-[#1E2EDE] text-[#FDFDFD] font-bold rounded-xl text-center shadow-lg"
                                onClick={() => setIsMobileMenuOpen(false)}
                            >
                                Get Started
                            </Link>
